...
class AppData {
  ...
  // AI REQUESTER - Switched back to gemini-pro which is stable on v1beta
  static Future<String> askGemini(String prompt) async {
    ...
    try {
      // URL FIX: Using gemini-pro on v1beta
  ...
  // EXAM GENERATOR - 30 Questions
  static Future<List<Map<String, dynamic>>> fetchMockTest() async {
    // Requesting 30 questions. Note: Response might be slow or truncated if too long, so we ask for concise JSON.
    final prompt = "Generate 30 multiple-choice questions on English Grammar (Tenses, Verbs, Prepositions). Output strictly a JSON array with keys: 'q', 'ans', 'options' (list of 4 strings). Do not use markdown code blocks.";
    ...
    return [
      {"q": "Error loading Exam. Check Internet.", "ans": "Retry", "options": ["Retry", "Check Net", "Exit", "Wait"]},
    ];
  }
}
    ...
    if (!isOnline && mounted) {
      showDialog(
        ...
        builder: (c) => AlertDialog(
          ...
          content: const Text("Please enable Mobile Data or Wi-Fi to use the AI features."),
          actions: [
            TextButton(
              onPressed: () {
                Navigator.pop(c);
...
class _ReadingStudioState extends State<ReadingStudio> {
  ...
  void _generatePassage() async {
    setState(() => _loading = true);
    final prompt = "Write a short story (approx 150 words). Provide 2 multiple choice questions. Output JSON keys: title, body, qa: [{q, a}]. No markdown.";
    final res = await AppData.askGemini(prompt);
...
class _ExamHallState extends State<ExamHall> {
  ...
  void _startExam() async {
    ...
    setState(() { _questions = q; _submitted = false; _answers.clear(); _seconds = q.length * 2 * 60; }); // 2 mins per question
